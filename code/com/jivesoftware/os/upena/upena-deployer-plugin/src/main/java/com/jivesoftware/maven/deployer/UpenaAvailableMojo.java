 ...
 * @phase validate
 */
public class UpenaAvailableMojo extends AbstractMojo {
            ...
            try {
                ...
                if (results != null && !results.isEmpty()) {
                    for (Entry<String, TimestampedValue<Instance>> e : results.entrySet()) {
                        ...
                        Instance gotInstance = requestHelper.executeRequest(new InstanceKey(e.getKey()), "/upena/instance/get", Instance.class, null);
                        if (gotInstance != null) {
                            Host gotHost = requestHelper.executeRequest(e.getValue().getValue().hostKey, "/upena/host/get", Host.class, null);
                            if (gotHost != null) {
                                getLog().info("AVAILABLE: " + serviceName + "." + gotInstance.instanceId
                                        + " running on host: " + gotHost.hostName + ":" + gotInstance.ports.get("main").port);
                            }

                        } else {
                            throw new MojoFailureException("Failed to get instance for service:" + serviceName);
                        }
                    }
                } else {
                    getLog().info(" No instances of " + serviceName + " have been deployed for.");
                }
            } catch (Exception x) {
